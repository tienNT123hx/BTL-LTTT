...
from huffman import makeFrequencyLibrary, makeHeap, makeHuffmanTreeFromHeap, generateHuffmanCodes, encodeText, decodeText, calculateMetrics
...
def encode_message():
    ...
    frequencyTable = {key: value /lenText  for key, value in frequency.items()}

    avg_code_length, entropy, compression_ratio, optimality_ratio = calculateMetrics(text,huffman_codes)
    output_text.config(state=tk.NORMAL)
    output_text.delete("1.0", tk.END)
    output_text.insert(tk.END, f"Input Text: {text}\nHuffman Codes: {huffman_codes}\nEncoded Message: {encoded_message}\nAverage Code Length: {avg_code_length}\nEntropy: {entropy}\nCompression ratio: {compression_ratio}\nOptimality ratio: {optimality_ratio}\nFrequency dictionary: {frequencyTable}\n")
...
def decode_message():
    ...
    for bit in encoded_text:
        if bit != "0" and bit != "1":
            messagebox.showerror("Error", "Please enter proper encoded text. (only contains 0 and 1)")
            return
